...
def _assert_siblings_disjoint(
    test: unittest.TestCase, forest: SuccessorForest, intervals: DFSIntervals
) -> None:
    normalized = forest.normalized
    for parent, children in forest.children.items():
        ...
        ranges.sort()
        for (l1, r1), (l2, r2) in itertools.pairwise(ranges):
