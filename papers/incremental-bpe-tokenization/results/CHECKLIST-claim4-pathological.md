# Reproducibility checklist — claim4-pathological

Per `docs/REPRODUCIBILITY_CHECKLIST.md`. Auto rows come from
[`../benchmarks/runs/claim4-pathological/environment.json`](../benchmarks/runs/claim4-pathological/environment.json).
...
| Exact command(s) executed | `uv run python papers/incremental-bpe-tokenization/benchmarks/claim4_pathological.py` | manual |
| Expected output | flat ~1.9 us/byte incremental (slope 0.995); quadratic restart baseline (slope 1.901); see results.json | manual |
| Warm-up / repeats / inner iterations | 3 / 10 / 1 (in results.json per entry) | manual |
| **Data** | | |
| Dataset name + version | N/A — synthetic input, generated in-script ('a'*n scaling shape (incremental vs restart-per-byte vs batch oracle)) | manual |
