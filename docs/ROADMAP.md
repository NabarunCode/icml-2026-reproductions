...
| 4. Implement | ◐ | Core incremental search implemented in Python 2026-07-18 (`experiments/incremental-bpe-tokenization/`) — correctness-focused first pass: Definition 4.1 checked directly via ancestor walk, not yet the O(1) DFS-interval/Centroid-Decomposition/Aho-Corasick speedups (documented gap, see the experiment's `README.md`). 12/12 tests pass: real reference-implementation traces, 200 randomized differential tests vs. an oracle, and an embedded empirical check of Theorem 4.2's Upward Closure on every byte fed. Eager output (§6) not started. |
