...
| 5. Benchmark | ◐ | First protocol-compliant runs done 2026-07-18 (clean tree, raw samples + environment.json committed): Claim 4 scaling shape (ours linear slope 0.995 vs restart-baseline quadratic 1.901 — see `papers/incremental-bpe-tokenization/results/claim4-pathological.md`) and the missing-centroid cost (tail latency linear in depth — `results/claim2-depth-scaling.md`). Blocked pieces: direct tiktoken baseline (encoding CDN blocked by sandbox egress) and Claim 3 real-vocab runs (huggingface.co blocked; needs user-provided tokenizer files or HF Jobs). HF Jobs GPU/compute credit available via `ICML-2026-agent-repro` org. |
