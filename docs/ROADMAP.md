...
| 4. Implement | ◐ | Core incremental search implemented in Python 2026-07-18 (`experiments/incremental-bpe-tokenization/`). Second pass same day added a real Aho-Corasick automaton, a tree-walk search (replacing brute-force length scanning), and eager output — all three mechanisms Claim 2 names now exist and are tested (17/17 tests pass). Remaining gap, clearly scoped: the O(1) DFS-interval test + Centroid Decomposition (§4.3/§5.3) — tree navigation is real but not yet centroid-balanced, so not yet O(log²t) on adversarial deep dictionaries (Appendix J's construction). See `experiments/incremental-bpe-tokenization/README.md` "Limitations" for the full, current gap list. |
